...
OrbitAggregate::OrbitAggregate(orbital_partition& orbits, HighsLp& original_lp,
...
void OrbitAggregate::buildResidualRows(){
    ...
    parent_row_.resize(agg_lp_.num_aggregate_cols_);
    ...
    for (res_row = 0; res_row < agg_lp_.num_residual_rows_; ++res_row){
        ...
        is_parent_.at(par) = 1;
        parent_row_.at(par) = agg_lp_.num_aggregate_rows_ + res_row;
        is_child_.at(chi) = 1;
...
void OrbitAggregate::aggregateAMatrix(){
    ...
    for (i_col = 0; i_col < agg_lp_.num_aggregate_cols_; ++i_col){
        ...
        for (i_mat = original_lp_.a_matrix_.start_.at(el); 
            ...
            HighsInt row_idx = orbits_.orbit.at(a_idx + original_lp_.num_col_) - agg_lp_.num_col_;
        ...
        if (agg_lp_.num_residual_cols_ && is_parent_.at(el)){
            agg_lp_.a_matrix_.value_.push_back(1);
            agg_lp_.a_matrix_.index_.push_back(parent_row_.at(el));
        }
        if (agg_lp_.num_residual_cols_ && is_child_.at(el)){
            agg_lp_.a_matrix_.value_.push_back(-1);
            agg_lp_.a_matrix_.index_.push_back(child_row_.at(el));
        }
