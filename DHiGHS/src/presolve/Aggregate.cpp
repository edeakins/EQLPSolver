...
HighsAggregate::HighsAggregate(HighsLp& lp, const struct eq_part& ep, HighsSolution& solution, HighsBasis& basis){
  ...
  nonBasicCol.resize(numCol + numRow);
  ...
  rowSense_.resize(numRow + maxLinkCols);
  rowIdx_.resize(numRow);
  ...
  newRowLifted.assign(numRow, false);
  tRhs.assign(numRow, 0);
...
void HighsAggregate::update(const struct eq_part& ep, const HighsSolution& solution, 
  ...
  // Previous solution
	...
	row_status = (basis.row_status);
  // Previous tableau
  tStart = tableau.Astart;
  tIndex = tableau.Aindex;
  tValue = tableau.Avalue;
  tBasicIndex = tableau.basicIndex;
  tBasicValue = tableau.basicValue;
  tReps = tableau.reps;
  tNumCol = tableau.numCol;
  tNumRow = tableau.numRow;
  tNumRCol = tableau.numRCol;
  tNumSCol = tableau.numSCol;
  tNumXCol = tableau.numXCol;
  reset();
  translateFrontsToColors();
  packVectors();
  scanLifts();
...
void HighsAggregate::translateFrontsToColors(){
  ...
  alp->numXCol_ = numCol_;
  alp->numSCol_ = numRow_;
  alp->numRCol_ = 0;
  alp->numRow_ = numRow_;
  alp->cell = cell;
  alp->labels.assign(partition.labels, partition.labels + numTot);
  alp->fronts = cellFront;
}
...
void HighsAggregate::scanLifts(){
  int i, rep, c, front, label;
  int offset, rowIdx, cnt = 0;
  for (i = 0; i < tReps.size(); ++i){
    rep =  tReps[i];
    c = cell[rep];
    front = cellFront[c];
    rep = partition.labels[front];
    isLifted[rep] = true;
    if (tReps[i] >= numCol) ++nRowCnt;
  }
  // std::cout << rowCnt << std::endl;
  for (i = numCol_; i < numCol_ + numRow_; ++i){
    rep = partition.labels[cellFront[i]];
    if (isLifted[rep]){
      newRowLifted[lRowCnt] = true;
      tRhs[lRowCnt] = tBasicValue[lRowCnt];
      rowIdx_[rep - numCol] = lRowCnt++;
    }
...
void HighsAggregate::foldMatrix(){
  int i, j, oldCol, colRep, rowRep, rowIdx, sIdx;
  int c, nAddtions = 0;
  ...
  for (i = 0; i < numCol_; ++i){
    ...
    // Do lifted rows first as they will be the first m' rows
    for (j = 0; j < lRowCnt; ++j){
      query = std::pair<int, int>(oldCol, j);
      rowCol = tEntry.find(query) != tEntry.end();
      if (rowCol){
        num = cellSize[cell[colRep]];
        den = previousCellSize[previousCell[colRep]];
        scale = tEntry[query];
        coeff[j] = scale*(num/den);
        ++nAddtions;
      }
    }
    for (j = Astart[colRep]; j < Astart[colRep + 1]; ++j){
      c = AindexPacked_[j] + numCol_;
    ...
    alp->Avalue_.resize(alp->Avalue_.size() + nAddtions);
    alp->Aindex_.resize(alp->Aindex_.size() + nAddtions);
    for (j = 0; j < coeff.size(); ++j){
      if (coeff[j]){
        // c = j + numCol_;
        // rowRep = partition.labels[cellFront[c]];
        lifted = newRowLifted[j];
        alp->Avalue_[alp->nnz_] = coeff[j] * (lifted ? 1 : cellSize[i]);
        alp->Aindex_[alp->nnz_++] = j;
        coeff[j] = 0;
      }
    }
...
void HighsAggregate::liftTableau(){
  int i, j, rep, front, offset, c, cnt = 0;
  std::pair<int, int> key;
  for (i = 0; i < tNumCol; ++i){
    for (j = tStart[i]; j < tStart[i + 1]; ++j){
      key = std::pair<int, int>(i, tIndex[j]);
      tEntry[key] = tValue[j];
    }
  }
}
...
void HighsAggregate::reset(){
  int i;
  alp->nnz_ = 0;
  previousNumCol_ = alp->numCol_ - alp->numRow_ - numLinkers_;
  for (i = 0; i < numTot; ++i){
  ...
  numLinkers_ = 0, lRowCnt = 0, nRowCnt = 0;
  tEntry.clear();
}
...
void HighsAggregate::foldRhs(){
  int i, rep, oldRowCell, cellIdx, rhs, rowIdx;
  for (i = 0; i < numRow_; ++i){
    rep = partition.labels[cellFront[i + numCol_]] - numCol;
    rowIdx = rowIdx_[rep];
    rhs = std::min(std::fabs(rowLower[rep]), std::fabs(rowUpper[rep]));
    if (std::abs(rowLower[rep]) == rhs)
      ...
      rowSense_[rowIdx] = "=";
    if (newRowLifted[rowIdx]) alp->rowLower_[rowIdx] = alp->rowUpper_[rowIdx] = tRhs[rowIdx];
    else alp->rowLower_[rowIdx] = alp->rowUpper_[rowIdx] = rhs * cellSize[i + numCol_];
    // std::cout << "row rhs set" << std::endl;
...
void HighsAggregate::setRowBasis(){
  ...
  for (i = 0; i < numRow_; ++i){
    ...
    //     row_status[rowIdx] == HighsBasisStatus::NONBASIC))//!nonBasicRow[rowIdx]){
...
void HighsAggregate::foldBnds(){
  ...
  // for (int i = 0; i < numCol_ + numRow_; ++i){
  //   alp->colUpper_[i] = 
  // }
}
...
void HighsAggregate::identifyLinks(){
  ...
  for (i = 0; i < numCol; ++i){
    if (partition.parents[i] >= 0){
      ...
      // if (alp->colUpper_[x1] == alp->colLower_[x1] &&
      //     alp->colUpper_[x2] == alp->colLower_[x2])
      //     continue;
