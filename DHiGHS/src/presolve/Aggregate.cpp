...
void HighsAggregate::setUp(const struct lpPartition* ep, HighsLp* lp){
  elp = lp;
  partition = ep;
  elpNnz_ = elp->nnz_;
  elpNumCol_ = elp->numCol_;
  elpNumRow_ = elp->numRow_;
  elpNumTot_ = elpNumCol_ + elpNumRow_;
}
...
// !!Test!! testing retract folding method
void HighsAggregate::foldRetract(){}
